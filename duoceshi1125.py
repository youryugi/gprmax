...
# GPR 固定不动：src_steps / rx_steps 设为 0 0 0
BASE_HEADER = """\
...
#material: 10      0.0002  1 0 wood_dry
...
def main():
    ...
    ap.add_argument("--max-blocks", type=int, default=1, help="max wood blocks per scene")
    ap.add_argument("--seed", type=int, default=123, help="random seed")
    ap.add_argument("--runs", type=int, default=1, help="B-scan traces per scene (-n)")
    ...
    ap.add_argument("--frames", type=int, default=100,
                    help="number of slow-time frames per scene (object motion steps)")
    ap.add_argument("--slow-dt", type=float, default=0.05,
                    help="slow-time interval between frames in seconds")
    ap.add_argument("--vx-min", type=float, default=-0.5,
                    help="min initial velocity in x (m/s) for wood blocks")
    ap.add_argument("--vx-max", type=float, default=0.5,
                    help="max initial velocity in x (m/s) for wood blocks")
    ap.add_argument("--ax-min", type=float, default=-0.5,
                    help="min acceleration in x (m/s^2) for wood blocks")
    ap.add_argument("--ax-max", type=float, default=0.5,
                    help="max acceleration in x (m/s^2) for wood blocks")
    ...
    # Generate scenes (每个scene下有若干frames)
    inpaths = []
