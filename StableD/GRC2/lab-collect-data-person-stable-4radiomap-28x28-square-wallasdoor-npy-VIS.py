...
def visualize_layout(walls, routers, x_coords, y_coords, title, save_path):
    ...
    for i, (rx, ry) in enumerate(routers):
        ax.scatter(rx, ry, marker='s', color='red', s=100, label=f"Router {i + 1}" if i == 0 else None)

    # 画出墙壁
    for wall in walls:
        (x1, y1), (x2, y2) = wall
        ax.plot([x1, x2], [y1, y2], 'black', linewidth=2, label="Walls" if wall == walls[0] else None)
...
print(f"所有布局已保存至 {layout_folder} 文件夹。")