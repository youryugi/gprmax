...
y = 0.60       # 同一深度
r = 0.05       # 同一半径
x0 = 0.60      # 起始 x (修改为 0.60，让第一个目标出现得晚一点，留出左侧空间)
dx = 0.40      # 间距
...
if __name__ == "__main__":
    ...
    try:
        # 调用 nvidia-smi 显示显卡信息
        subprocess.run(["nvidia-smi", "--query-gpu=index,name,memory.total", "--format=csv,noheader"], check=True)
        print("------------------------------------------------")
    ...
    # 请根据实际情况修改这里！
    AVAILABLE_GPUS = [0, 1, 2]  # 修改为使用 3 个 GPU
    
    N_TRACES = 115 # 增加道数，扫描更长的距离 (约 2.3m)
    VARIATIONS_PER_CASE = 50  # 修改为每种情况生成 50 个变体
    # ===========================================
